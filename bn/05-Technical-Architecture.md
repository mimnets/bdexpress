...
| supplier_info | JSON | সরবরাহকারীর বিবরণ |
| created_at | Timestamp | |
...
| verified_by | UUID | যিনি যাচাই করেছেন |
| created_at | Timestamp | |
...
### পাবলিক
```
GET  /api/products              — নির্বাচিত পণ্যের তালিকা
GET  /api/products/:id          — পণ্যের বিস্তারিত
POST /api/quote                 — লিংক থেকে কোট পান
GET  /api/shipping-cost         — শিপিং খরচের অনুমান
POST /api/auth/register         — নিবন্ধন
POST /api/auth/login            — লগইন
GET  /api/categories            — পণ্যের ক্যাটাগরি
```
...
PUT  /api/admin/orders/:id      — অর্ডারের অবস্থা আপডেট
POST /api/admin/products        — নির্বাচিত পণ্য যোগ
...
| **পেমেন্ট নিরাপত্তা** | SSL/TLS, SSLCommerz সংযুক্তিকরণ (কার্ড সংরক্ষণ নেই) |
...
| **SQL ইনজেকশন** | Prisma-র মাধ্যমে প্যারামিটারাইজড কোয়েরি |
| **DDoS** | Cloudflare WAF |
| **ব্যাকআপ** | প্রতিদিন স্বয়ংক্রিয় DB ব্যাকআপ |
| **মনিটরিং** | ত্রুটির জন্য Sentry, আপটাইম মনিটরিং |

## মোবাইল অ্যাপ কাঠামো

### ক্রস-প্ল্যাটফর্ম পদ্ধতি: React Native (Expo)
- **এক কোডবেস** iOS + Android-এর জন্য
- **Expo SDK** ক্যামেরা, পুশ নোটিফিকেশন, পেমেন্টের জন্য
- **নেটিভ মডিউল** দরকারে bKash SDK সংযুক্তিকরণের জন্য
- **কোড শেয়ারিং** ওয়েবের সাথে যেখানে সম্ভব (API লেয়ার)
- **OTA আপডেট** EAS-এর মাধ্যমে (ছোট ফিক্সের জন্য অ্যাপ স্টোর দেরি নেই)

### অ্যাপের স্ক্রিন (এমভিপি)
১. **হোম** — সেবার সারসংক্ষেপ, কীভাবে কাজ করে, CTA
২. **পণ্য খোঁজা/ক্যাটালগ** — নির্বাচিত পণ্য ব্রাউজ + খোঁজা
৩. **লিংক জমা ফর্ম** — Taobao/1688 URL পেস্ট করুন
৪. **কোট ফলাফল** — আনুমানিক খরচ ভেঙে দেখা
৫. **চেকআউট** — ঠিকানা, পেমেন্ট পদ্ধতি
৬. **অর্ডার ট্র্যাকিং** — রিয়েল-টাইম অবস্থা
৭. **প্রোফাইল** — অর্ডার, ঠিকানা, সেটিংস
৮. **নোটিফিকেশন** — পুশ নোটিফিকেশনের তালিকা
