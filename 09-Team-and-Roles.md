# 09 — Team & Roles

[← Home](./) | [Document Index](./INDEX)
🌐 [বাংলা](./bn/09-Team-and-Roles)

## Core Team

### 1. Mr. Touab — China Operations & Sourcing Lead

**Background:** Vast knowledge of China import business, complete business model for China import, helps BD e-commerce companies & SMEs, has China warehouse

**Responsibilities:**
- China warehouse operations & management
- Supplier relationship management (Taobao/1688/Alibaba)
- Product sourcing & procurement
- Quality control at warehouse
- International shipping coordination
- Customs documentation (China side)
- Negotiating wholesale pricing with Chinese suppliers
- Managing China-based staff

**Decision-making authority:**
- China supplier selection & pricing
- Warehouse operational decisions
- Shipping method recommendations

**Tools they'll use:**
- Admin dashboard (order notifications)
- WhatsApp/Telegram for real-time team communication
- Spreadsheet/dashboard for inventory tracking

---

### 2. Mr. Sabbir — Local Operations & Customer Experience Lead

**Background:** Accountant background, expert in mobile/computer/gadgets/tech products, knows BD local market demands, customer/user experience engagement

**Responsibilities:**
- Company registration & legal compliance (BD)
- Office setup & management in Uttara
- Last-mile delivery partner management
- Customer support team management
- Payment verification & financial tracking
- Product catalog curation (especially gadgets/electronics)
- Customer experience & satisfaction
- Content creation (Bengali — educational videos, social media)
- Local marketing & promotions

**Decision-making authority:**
- Customer support escalation handling
- Local logistics partner selection
- Pricing adjustments for BD market
- Marketing budget allocation

**Tools they'll use:**
- Admin dashboard
- Accounting software (Tally/ZohoBooks)
- Social media management tools
- Customer support platform
- Delivery partner dashboards

---

### 3. Monirul Islam (Monir) — Technology & Automation Lead

**Background:** Claude Code / vibe coding — develops e-commerce website + mobile app (iOS & Android), automates customer support, sales, social media

---

### 4. Mr. Akram — Logistics & Supply Chain Lead

**Background:** Expert in logistics & supply chain operations with deep experience in cross-border shipping, freight forwarding, customs clearance, and last-mile delivery networks

**Responsibilities:**
- International shipping route planning & carrier selection
- Freight forwarding partnerships (air, sea, express)
- Customs clearance process & broker management (Bangladesh side)
- Last-mile delivery partner evaluation & onboarding
- Shipping cost optimization & rate negotiation
- Supply chain workflow design from China warehouse to BD doorstep
- Shipment tracking & exception handling
- Logistics performance metrics & reporting

**Decision-making authority:**
- Carrier & freight forwarder selection
- Shipping method recommendations per order type
- Last-mile delivery partner selection
- Customs broker partnerships

**Tools they'll use:**
- Admin dashboard (logistics tracking module)
- Freight comparison tools
- Customs documentation software
- WhatsApp/Telegram for real-time coordination
- Spreadsheet/dashboard for shipping metrics

---

**Responsibilities:**
- Full-stack development (website, mobile app, backend)
- AI/automation implementation
- Payment gateway integration
- API integration (Taobao, 1688, logistics)
- Infrastructure & hosting management
- Security & data protection
- Technical team hiring & management
- Product roadmap & prioritization
- Analytics & reporting systems

**Decision-making authority:**
- Technology stack & architecture decisions
- Feature prioritization
- Third-party tool selection (tech)
- Data & analytics strategy

**Tools they'll use:**
- VS Code / Cursor with Claude Code
- GitHub for version control
- Vercel / Supabase / Cloudflare
- React Native / Next.js
- AI tools for automation

---

## Phase 1 Hiring (Month 2–3)

| Role | Monthly Cost (BDT) | When | Reports To |
|------|-------------------|------|------------|
| Customer Support Agent (1) | 15,000–20,000 | Month 2 | Sabbir |
| Bangladesh Office Assistant (1) | 10,000–15,000 | Month 2 | Sabbir |
| China Warehouse Assistant (1) | 20,000–30,000 | Month 1 | Touab |
| Part-time Content Creator (1) | 10,000–15,000 | Month 3 | Sabbir |

## Phase 2 Hiring (Month 4–6)

| Role | Monthly Cost (BDT) | When | Reports To |
|------|-------------------|------|------------|
| Junior Developer (1) | 30,000–50,000 | Month 5 | Monir |
| Full-time Customer Support (2) | 30,000–40,000 | Month 5 | Sabbir |
| Marketing Specialist (1) | 25,000–40,000 | Month 5 | Sabbir/Monir |
| Logistics Coordinator (1) | 20,000–30,000 | Month 4 | Touab |

## Communication Protocol

| Frequency | Meeting | Participants | Format |
|-----------|---------|-------------|--------|
| **Daily** | Quick standup | All 4 | WhatsApp/Telegram voice note or text |
| **Weekly** | Progress review | All 4 | Video call (60 min) |
| **Bi-weekly** | Strategic planning | All 4 | Video call (90 min) |
| **Monthly** | Financial review | All 4 | Video call + spreadsheet |

### Communication Tools
- **Daily chat:** WhatsApp group
- **File sharing:** Google Drive
- **Project management:** Notion / Linear / Trello
- **Code & tech:** GitHub
- **Video calls:** Google Meet / Zoom

## Equity & Profit Sharing (To Discuss)

> **⚠️ OPEN QUESTION:** Need to discuss how equity/profits are split between the 4 partners. Suggested structure for discussion:
> - Based on contribution (time, resources, existing assets)
> - Vesting schedule (e.g., 2-year cliff, 4-year vest)
> - Profit distribution vs reinvestment policy

Suggested discussion points:
1. Does Touab's existing China warehouse & network count as capital contribution?
2. Does Sabbir's time for operations count as sweat equity?
3. Does Monir's tech development count as sweat equity + cash cost?
4. Does Akram's logistics expertise & supply chain network count as sweat equity?
5. Will partners take salary from day 1, or reinvest all profits?
6. What percentage is reserved for future employees/investors?
-e 
---
[← Back to Home](./) | [Full Document Index](./INDEX)
🌐 [বাংলা](./bn/09-Team-and-Roles)

